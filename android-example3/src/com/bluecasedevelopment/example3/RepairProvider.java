...
public class RepairProvider extends ContentProvider {
	...
	public int delete(Uri uri, String selection, String[] selectionArgs) {
		...
		if (count > 0) {
			this.sendChangedBroadcast(Intent.ACTION_DELETE, uri);
	...
	public Uri insert(Uri uri, ContentValues values) {
		...
		if (newId > -1) {
			...
			this.sendChangedBroadcast(Intent.ACTION_INSERT, result);
	...
	public int update(Uri uri, ContentValues values, String selection,
		...
		int count = db.update(Repair.TABLE_NAME, values, selection,
				selectionArgs);
		if (count > 0) {
			this.sendChangedBroadcast(Intent.ACTION_EDIT, uri);
			this.getContext().getContentResolver().notifyChange(uri, null);
		}
		return count;
	}

	private void sendChangedBroadcast(String action, Uri data) {
		Intent i = new Intent(action).setType(Repair.CONTENT_ITEM_TYPE);
		if (data != null)
			i = i.setData(data);
